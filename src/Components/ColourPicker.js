...
export const ColourPicker = (props) => {
  ...
  return (
    <div style={{width: '50vw'}} className='d-flex'>
        {modes.map((item, idx)=>{
            return(
                <div id={idx} style={{width: '1.6rem', height: '1.6rem', backgroundColor: NoteColourMapper(item.toLowerCase(), displayMode)}} className={`border border-${displayMode === "dark" ? "light" : "dark"} mx-2 rounded-circle ${activeColour === item.toLowerCase() ? `border-3`: 'border-1'}`} onClick={()=>{handleClick(item.toLowerCase())}} value={item.toLowerCase()}></div>
            )
        })}
    </div>
  );
